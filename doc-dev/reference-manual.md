...
COMMAND = setClock { off | <hour 0-23 (INT)> <minute 0-59 (INT)> [<second 0-59 (INT)>] }
...
- `setClock { off | <hour> <minute> [second] }` syncs the UHK60 segment display clock to the provided 24-hour time and alternates between hour and minute text, such as `14H` and `37M`.
  - The clock is inferred from firmware uptime after the latest sync, so a host-side script should re-sync periodically to compensate for drift and after reconnects.
  - `setClock off` disables the clock display and returns to normal keymap abbreviation display.
  - This command is only implemented on UHK60.
